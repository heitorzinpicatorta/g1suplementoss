...
export function Home() {
  ...
  const handleAddToCart = useCallback((product: any, qty = 1) => {
    setCartItems((prev) => {
      ...
      return [...prev, { id: product.id, name: product.name, image: product.image, priceTo: product.priceTo, qty }];
    });
  ...
  const filteredProducts = useMemo(() => {
    ...
    let list = products.filter((p) => {
      ...
      const matchesMin = minVal === null || p.priceTo >= minVal;
      const matchesMax = maxVal === null || p.priceTo <= maxVal;
    ...
    <div className="min-h-[100dvh] flex flex-col" style={{ background: bg }}>
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full px-6 py-12">
        ...
        {filteredProducts.length === 0 ? (
          ...
          <>
            {/* Grid plano — sem agrupamento por categoria */}
